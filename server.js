...
const headerHtml = `
  ...
  <style>
    ...
    .player-list li { padding: 15px; border-bottom: 1px solid #eee; font-size: 1.1rem; display: flex; align-items: center; }
    .player-list a { text-decoration: none; color: #007bff; flex-grow: 1; }
...
app.get("/", ensureAuthenticated, async (req, res) => {
  ...
  const searchQuery = (req.query.q || "").trim(); // 名前検索
  const numQuery = (req.query.num || "").trim(); // 背番号検索
  ...
  try {
    ...
    $("table.rosterlisttbl").each((_, table) => {
      ...
      $(table).find("tr").each((i, el) => {
        ...
        if ($(el).hasClass("rosterPlayer")) {
          ...
          const link = nameContainer.find("a").attr("href") || null;
    ...
    // フィルタリング処理（ポジション、名前、背番号）
    let filteredPlayers = players;
    if (filterPos) filteredPlayers = filteredPlayers.filter(p => p.position === filterPos);
    if (searchQuery) filteredPlayers = filteredPlayers.filter(p => p.name.includes(searchQuery));
    if (numQuery) filteredPlayers = filteredPlayers.filter(p => p.number === numQuery);
    ...
    let html = headerHtml + pageHeader + `<h1 style="text-align:center;">${teams[teamCode].name}</h1><a href="/" class="back-link">← チーム選択へ戻る</a>
      <div class="card">
        <form method="get" action="/">
          ...
          <input type="text" name="q" placeholder="名前で検索" value="${searchQuery}">
    ...
    if (filteredPlayers.length === 0) {
      html += `<li style="justify-content:center; color: #999;">該当する選手が見つかりませんでした</li>`;
    } else {
      filteredPlayers.forEach(p => {
        html += `<li>
          <span style="width:50px;font-weight:bold;color:#666;">${p.number}</span>
          ${p.link ? `<a href="/player?team=${teamCode}&direct=${encodeURIComponent(p.link)}&num=${p.number}">${p.name}</a>` : `<span>${p.name}</span>`}
          <span style="font-size:0.8rem; color:#888; margin-left:10px;">${p.position}</span>
        </li>`;
      });
    }
    res.send(html + "</ul>");
  } catch (err) { res.send("データの取得に失敗しました。"); }
});
...
app.get("/player", ensureAuthenticated, async (req, res) => {
  ...
  try {
    ...
    // 選手名取得の修正
    let targetName = $p("#pc_v_name #nm_kanji").text().trim() || $p("h1").text().trim();
    targetName = targetName.replace("日本野球機構オフィシャルサイト", "").trim();
  ...
  } catch (err) { res.send("詳細情報の取得に失敗しました。"); }
});
